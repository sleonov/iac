resource "aws_route53_zone" "private_east" {
  provider = aws.east
  name     = local.east_private_zone_name
...
# Cross-region association — allows west VPC resources to resolve this zone via VPC peering.
resource "aws_route53_zone_association" "private_east_to_west" {
  provider   = aws.east
  zone_id    = aws_route53_zone.private_east.zone_id
