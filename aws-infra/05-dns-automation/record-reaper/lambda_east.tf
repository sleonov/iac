resource "aws_cloudwatch_log_group" "record_reaper_east" {
  provider          = aws.east
  name              = "/aws/lambda/dns-record-reaper"
  retention_in_days = var.log_retention_days
}

resource "aws_lambda_function" "record_reaper_east" {
  provider         = aws.east
  ...
  timeout          = 60
  depends_on       = [aws_cloudwatch_log_group.record_reaper_east]
}
