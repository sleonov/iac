...
resource "aws_cloudwatch_event_rule" "record_reaper_west" {
  provider            = aws.west
...
resource "aws_cloudwatch_event_target" "record_reaper_west" {
  provider  = aws.west
  rule      = aws_cloudwatch_event_rule.record_reaper_west.name
  target_id = "dns-record-reaper"
  arn       = aws_lambda_function.record_reaper_west.arn
}
...
resource "aws_lambda_permission" "eventbridge_west" {
  ...
  function_name = aws_lambda_function.record_reaper_west.function_name
  principal     = "events.amazonaws.com"
  source_arn    = aws_cloudwatch_event_rule.record_reaper_west.arn
}
