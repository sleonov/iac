...
Dependencies in the table below are derived from `data "terraform_remote_state"` blocks in each module's `main.tf` — if a module declares a remote state source, that source must be applied first or the plan will fail.
